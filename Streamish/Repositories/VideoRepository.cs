    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                            SELECT v.Title, v.Description, v.Url, v.DateCreated, v.UserProfileId, up.DateCreated AS UserDateCreated, up.Email, up.ImageUrl, up.Name
                            FROM Video v
                            JOIN UserProfile up ON v.UserProfileId = up.Id
                            WHERE v.Id = @id";
            ...
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                        ...
                        up.DateCreated AS UserDateCreated, up.Email, up.ImageUrl, up.Name,
                        c.Id AS CommentId, c.Message, c.UserProfileId AS CommentUserProfileId
                            FROM Video v
                            JOIN UserProfile up ON v.UserProfileId = up.Id
                            LEFT JOIN Comment c ON c.VideoId = v.Id
        ...
        public List<Video> SearchByDate(DateTime since, bool sortDesc)
        {
            using (var conn = Connection)
            {
                conn.Open();
